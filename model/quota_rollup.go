...
func ensurePostgresQuotaDataRollups(db *gorm.DB) error {
	statements := []string{
		`CREATE UNIQUE INDEX IF NOT EXISTS uq_quota_data_hour ON quota_data (user_id, username, model_name, created_at)`,
		`CREATE UNIQUE INDEX IF NOT EXISTS uq_quota_data_daily_bucket ON quota_data_daily (user_id, username, model_name, created_at)`,
		`CREATE UNIQUE INDEX IF NOT EXISTS uq_quota_data_monthly_bucket ON quota_data_monthly (user_id, username, model_name, created_at)`,
...
EXECUTE FUNCTION sync_quota_data_rollups()`,
	...
	for _, statement := range statements {
		if err := db.Exec(statement).Error; err != nil {
			return fmt.Errorf("failed to ensure PostgreSQL quota data rollups: %w", err)
		}
	}
	return nil
}
