...
- 当前分支默认通过内嵌 SQL 迁移自动维护 PostgreSQL 结构、汇总回填和性能索引；详见 [PostgreSQL 性能与索引策略](./postgresql-performance.md)。
- k3s 默认设置 `POSTGRES_LOG_PARTITIONING=auto`。新库会自动创建月度分区日志表；已有普通日志表会在后端启动迁移阶段自动转换为分区表。
