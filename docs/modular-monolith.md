...
控制面以单条 MVCC 查询捕获渠道和能力并发布版本快照；数据面选路、模型列表、能力投影与别名不再查询渠道配置表。保留旧快照、乐观状态纠正和可取消的通知/补偿同步。完整测试、build/vet、无渠道配置读取权限的数据面实际鉴权转发及重启验证通过。插件配置、渠道写入/多 Key 运行状态和结算仍待收敛，详见 [控制面与数据面拆分](control-data-plane.md)。
...
暂存中断的渠道状态批量读取优化，恢复已提交的运行状态实现；修正镜像和发布流程的版本号注入路径。版本路径改动提交为 `381f463e6`。按用户后续指令，不继续构建或运行验收。

### 第六十一批：账务与请求配置一致性（未验证）

请求账务改为持久化生命周期和幂等业务凭证；资金、Token、统计及任务标记按事务共同提交，控制面恢复明确的待处理意图。异步结果不明时保留预扣供核对。请求价格、表达式与转换配置绑定发布快照，异步任务使用持久化计价上下文。Realtime 改为累计预留与最终结算。相关代码和回归用例已编写，但按用户要求尚未执行验证，详见 [控制面与数据面拆分](control-data-plane.md)。
