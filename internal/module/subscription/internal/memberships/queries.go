...
func (s *Store) GetAllActiveUserSubscriptions(ctx context.Context, userId int) ([]SubscriptionSummary, error) {
	...
	err := s.db.WithContext(ctx).Where("user_id = ? AND status = ? AND end_time > ?", userId, "active", now).
...
func (s *Store) HasActiveUserSubscription(ctx context.Context, userId int) (bool, error) {
	if userId <= 0 {
...
func (s *Store) UserActiveSubscriptionsAllowWalletOverflow(ctx context.Context, userId int) (bool, error) {
	if userId <= 0 {
		return false, errors.New("invalid userId")
	}
	now := common.GetTimestamp()
...
func (s *Store) GetAllUserSubscriptions(ctx context.Context, userId int) ([]SubscriptionSummary, error) {
	if userId <= 0 {
		return nil, errors.New("invalid userId")
	}
	var subs []UserSubscription
