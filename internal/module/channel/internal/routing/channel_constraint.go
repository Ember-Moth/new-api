...
// Caller must hold r.channelSyncLock (read lock). The input slice is never mutated.
// A missing id in r.channelsIDM is kept for request_path (downstream consistency
// error) and dropped for task_plugin_identity, matching the previous filters.
func (r *Runtime) filterCandidateIDs(ids []int, modelName string, filters []dto.ChannelFilter) (kept []int, emptiedBy dto.ChannelFilterKind) {
	if len(ids) == 0 {
		return ids, ""
	}
	kept = ids
	for _, kind := range filterEvalOrder {
		...
		for _, id := range kept {
			channel, exists := r.channelsIDM[id]
