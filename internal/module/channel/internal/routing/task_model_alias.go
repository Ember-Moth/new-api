...
func (r *Runtime) ResolveTaskModelAlias(g *jsplugin.RoutingGeneration, name string) (TaskAliasTarget, bool) {
	if g == nil || name == "" {
		return TaskAliasTarget{}, false
	}
	view := r.loadFreshTaskAliasView(g)
	if view == nil {
		return TaskAliasTarget{}, false
	}
...
func (r *Runtime) loadFreshTaskAliasView(g *jsplugin.RoutingGeneration) *taskAliasView {
	view := r.taskAliasViewPtr.Load()
	if taskAliasViewFresh(view, g.Number) {
		return view
	}
	r.taskAliasRebuildMu.Lock()
	defer r.taskAliasRebuildMu.Unlock()
	view = r.taskAliasViewPtr.Load()
	if taskAliasViewFresh(view, g.Number) {
		return view
	}
	rebuilt := r.buildTaskAliasView(g)
	r.taskAliasViewPtr.Store(rebuilt)
	return rebuilt
}
...
func (r *Runtime) rebuildTaskAliasView() {
	r.taskAliasRebuildMu.Lock()
	defer r.taskAliasRebuildMu.Unlock()
	r.taskAliasViewPtr.Store(r.buildTaskAliasView(jsplugin.DefaultRegistry.Generation()))
}
...
func (r *Runtime) buildTaskAliasView(generation *jsplugin.RoutingGeneration) *taskAliasView {
	genNum := uint64(0)
	if generation != nil {
