			...
			if shouldSelectChannel {
				...
				if strings.HasPrefix(c.Request.URL.Path, "/pg/chat/completions") {
					...
					if playgroundRequest.Group != "" {
						if !identitygroups.GroupInUserUsableGroups(usingGroup, playgroundRequest.Group) && playgroundRequest.Group != usingGroup {
...
func SetupContextForSelectedChannel(c *gin.Context, channel *model.Channel, modelName string) *types.NewAPIError {
	...
	key, index, newAPIError := model.ChannelService().GetNextEnabledKey(channel)
	if newAPIError != nil {
		return newAPIError
	}
	...
	common.SetContextKey(c, constant.ContextKeyChannelKeyPoolFingerprint, model.ChannelKeyPoolFingerprint(channel))
