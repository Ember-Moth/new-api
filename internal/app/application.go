...
import (
	...
	billingpurchases "github.com/QuantumNous/new-api/internal/module/billing/purchases"
...
func Run(assets router.WebAssets) {
	...
	billingService := billing.New(billing.Dependencies{RewardConfig: rewardConfiguration, StatementConfig: statementConfiguration, RewardLog: func(ctx context.Context, id int, message string) {
		model.LogService().RecordLog(ctx, id, model.LogTypeSystem, message)
	}, PriceSync: pricesync.New(pricesync.Dependencies{Sources: pricingSyncSources(model.ChannelService()), Credential: model.ChannelService().PricingSyncCredential, LocalData: localPricingSyncData}), Pricing: model.PricingService(), Accounting: ledger, Webhooks: billingwebhooks.New(billingwebhooks.Dependencies{EpayVerifier: paymentGateway.VerifyEpay, Config: paymentWebhookConfig, TopUps: model.TopUpStore(), Subscriptions: model.SubscriptionPayments()}), TopUps: model.TopUpStore(), DB: model.DB, PaymentAllowed: operation_setting.IsPaymentComplianceConfirmed})
	identityService := identity.New(identity.Dependencies{Cache: common.RDB, Authentication: service.AuthenticationRuntime(), TwoFAEvent: func(id int, message string) { model.RecordLog(id, model.LogTypeSystem, message) }, VerifyEmail: func(email, code string) bool {
	...
	billingService.PaymentConfig = paymentconfig.New(paymentconfig.Dependencies{TermsVersion: operation_setting.CurrentComplianceTermsVersion, SaveOptions: model.OptionManager().UpdateOptionsBulk, Config: func() paymentconfig.Config {
