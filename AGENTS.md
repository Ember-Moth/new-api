...
- Data-plane task-plugin configuration must come from DragonflyDB snapshots, never the task_plugins table. Preserve batch generation installation and last-good behavior on compilation/load failures; initialize plugin configuration before starting system task execution.
- Multi-key polling cursors live only in DragonflyDB, scoped by channel and key-pool fingerprint. Never persist polling positions in channel_info or recreate multi_key_polling_index. Local key-status mutexes are process synchronization only.
- Automatic channel/key disable state lives only in DragonflyDB. Manual stops are persistent policy and take precedence. Never save automatic status/reason/time projections into PostgreSQL configuration. Carry the key-pool fingerprint captured with the selected request key through delayed outcomes; do not reload a newer channel to construct that identity.
- Wallet and token balances are authoritative PostgreSQL facts and commit synchronously. Batch statistics use durable quota_batch_deliveries; do not restore process-local pending accounting maps, cache-first financial mutations, or a second SQL fallback after uncertain enqueue errors.
- Data-plane channel selection, model lists, pricing abilities and task-alias views load from published DragonflyDB channel snapshots. Startup must fail when the initial snapshot cannot load; never repair abilities implicitly during startup. Preserve the last good routing snapshot on refresh failure.
- System option writes and snapshot publication must share the PostgreSQL advisory transaction lock in the option store. Data-plane options load only from versioned DragonflyDB snapshots; do not add a PostgreSQL fallback. Notifications are hints and must retain periodic reconciliation.
...
- Cache services use DragonflyDB. Application startup requires `REDIS_CONN_STRING`; all application planes share the same logical database. Node heartbeats and system-task execution leases live only in DragonflyDB and expire automatically; do not recreate PostgreSQL instance-status or lease tables. Durable task records retain the execution owner to fence late results; cache errors must not bypass lease checks. Email verification and password reset codes are shared single-use DragonflyDB challenges; do not reintroduce process-local verification maps or post-action unconditional deletion. OAuth/Passkey/2FA ceremony state lives only in DragonflyDB; do not recreate `auth_flows`. Provider assertion consumption is a durable security receipt in `auth_assertion_receipts` so cache loss cannot re-enable a spent signature. Business-action failures must not rearm consumed ceremony tokens. Login sessions, refresh rotation, revocation and issuance limits are authoritative DragonflyDB state; do not recreate `user_sessions` or add PostgreSQL fallback/cache refill paths. Session creation must enforce both limits atomically, and revocation/password resets must not erase issuance history. Keep the Redis-compatible client and `REDIS_CONN_STRING`/`REDIS_POOL_SIZE` configuration names. Exercise real DragonflyDB with `TEST_DRAGONFLY_DSN` for changes affecting Lua scripts, TTLs, transactions, rate limits, sessions, or quota caches; in-memory Redis test doubles alone do not establish DragonflyDB compatibility.
