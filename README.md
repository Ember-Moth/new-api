...
当前数据面仍访问 PostgreSQL 进行鉴权、计费和任务持久化。本批完成监听接口与维护任务的角色隔离；节点心跳、系统任务执行租约、邮箱验证码和 OAuth/Passkey/两步登录挑战已迁入 DragonflyDB；登录会话及其运行限制也已迁移；系统 options 已支持版本发布、通知和补偿核对；渠道/插件配置快照及可靠结算仍在后续实施中。部署边界及验证见 [控制面与数据面拆分](docs/control-data-plane.md)。
