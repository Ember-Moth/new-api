...
**缓存配置：**
- `REDIS_CONN_STRING`：DragonflyDB 缓存（推荐）
...
当前数据面仍访问 PostgreSQL 进行鉴权、计费和任务持久化。本批完成监听接口与维护任务的角色隔离；节点心跳、系统任务执行租约、邮箱验证码和 OAuth/Passkey/两步登录挑战已迁入 DragonflyDB；登录会话及其运行限制也已迁移；系统 options 已支持版本发布、通知和补偿核对；渠道路由也已使用版本快照；插件配置也已使用共享快照；运行状态收敛与可靠结算仍在后续实施中。部署边界及验证见 [控制面与数据面拆分](docs/control-data-plane.md)。
...
系统配置由控制面写入 PostgreSQL 后发布到 DragonflyDB。数据面通过只读快照初始化和更新配置，不读取 `options` 表。版本为内容摘要，通知丢失会按 `SYNC_FREQUENCY` 补偿；控制面周期性重建丢失的快照。系统实例信息的 `info.extra.options_version` 表示该进程已应用的版本。
