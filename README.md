...
当前数据面仍访问 PostgreSQL 进行鉴权、计费和任务持久化。本批完成监听接口与维护任务的角色隔离；节点心跳和系统任务执行租约已迁入 DragonflyDB；短期流程状态、配置主动发布和可靠结算仍在后续实施中。部署边界及验证见 [控制面与数据面拆分](docs/control-data-plane.md)。
